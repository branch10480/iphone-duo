...
1. **リポジトリの push** —— sandbox ではできないので通常環境で:
   ```sh
   cd ~/ghq/github.com/branch10480/iphone-duo
   gh repo create iphone-duo --public --push   # （private なら --private）
   ```
2. **サンプルアプリ本体の作成** —— ここからが次のセッションの本体。要件は `docs/iphone-duo-support.md` §6 のチェックリスト（14 項目）。
3. **内側ディスプレイの論理解像度をシミュレータ実測** —— 推測で残っている唯一の数字（§2、要実機確認の 1 つ）。
