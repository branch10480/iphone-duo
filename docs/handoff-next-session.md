...
- **Regions 図を pose 別 2 枚に（closed / open）（2026-09-24）**: 「図を閉じた/開いたそれぞれで書いてほしい」依頼に応じ、`.pose-row`（2 列 grid）+ `.pose` wrapper に 2 枚の `.demo-areas` を並べた。pose 別アスペクトを実測ディスプレイ比で `pose-closed`=280×420（外側 portrait 1398×2034、2:3）/ `pose-open`=480×360（内側 landscape 2853×2007、4:3）。occlusion 位置も pose 別に実測どおり（closed 幅18% / open 幅9%、共に y0〜25%）。division は closed=0件（描画しない）/ open=inactive（淡い中央縦バンド）。content 行は両 pose とも左側へ退避（displacement）。**縦（portrait）図の件**: 追加の第 3 枚は不要と判断した。closed（外側）のバッファが portrait 実測そのものなので、閉じた図=縦・開いた図=横で既に縦横両方向をカバー。
